 ...
 *
 * Ported from src/services/jensen.ts (renderer/browser WebUSB) to main process (node-usb).
 * ONLY the USB transport layer changed — all protocol logic is identical.
 */

// WebUSB types (USBDevice, USBInTransferResult, etc.) — needed for main process
// since tsconfig.node.json only includes "node" types
...
// node-usb may not export USBConnectionEvent — define locally
...
export class JensenDevice {
  ...
  static isSupported(): boolean {
    return webusb !== null && webusb !== undefined
  }
  ...
  async connect(signal?: AbortSignal): Promise<boolean> {
    ...
    try {
      picked = await webusb.requestDevice({
  ...
  private setupUsbDisconnectListener(): void {
    ...
    webusb.addEventListener('disconnect', this.usbDisconnectHandler)
  }

  private removeUsbDisconnectListener(): void {
    if (this.usbDisconnectHandler) {
      webusb.removeEventListener('disconnect', this.usbDisconnectHandler)
  ...
  setupUsbConnectListener(): void {
    ...
    webusb.addEventListener('connect', this.usbConnectHandler as EventListener)
  ...
  removeUsbConnectListener(): void {
    ...
    if (this.usbConnectHandler) {
      webusb.removeEventListener('connect', this.usbConnectHandler as EventListener)
    }
